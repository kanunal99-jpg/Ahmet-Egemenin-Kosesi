-- ====================================================================
-- Ahmet Egemen'in Köşesi - Final Database Schema & Security Protocol
-- ====================================================================
...
-- 2.8 Idempotent Column Additions for Pre-existing Tables
ALTER TABLE public.videos ADD COLUMN IF NOT EXISTS view_count INT NOT NULL DEFAULT 0;
ALTER TABLE public.parent_settings ADD COLUMN IF NOT EXISTS failed_attempts INT NOT NULL DEFAULT 0;
ALTER TABLE public.parent_settings ADD COLUMN IF NOT EXISTS locked_until TIMESTAMPTZ DEFAULT NULL;
...
-- 4.1 Profiles Policies (STRICT READ PRIVACY: Users can ONLY read their own profile)
...
CREATE POLICY "Users can insert their own profile"
  ...
  WITH CHECK (
    auth.uid() = id AND
    role IN ('child', 'parent') -- Prevents client self-assigning publisher or admin
  );
...
-- ONLY Publisher and Admin roles can insert videos (Child and Parent are REJECTED)
...
CREATE POLICY "Publishers and Admins can insert videos"
  ...
  WITH CHECK (
    ...
    EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = auth.uid() AND role IN ('publisher', 'admin')
    )
  );

-- ONLY Publisher (for own video) or Admin can update / soft-delete
...
CREATE POLICY "Publishers and Admins can update their own videos"
  ...
  USING (
    ...
    AND EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('publisher', 'admin'))
  )
  WITH CHECK (
    (owner_id = auth.uid() OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin'))
    AND EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('publisher', 'admin'))
...
-- 4.6 Parent Settings Policies
-- No direct RLS policies granted for parent_settings to direct client queries.
-- All parent_settings interactions MUST occur via SECURITY DEFINER RPC functions.

-- 5. RPC FUNCTIONS (WITH MANDATORY SET search_path = public, pg_temp)
...
BEGIN
  ...
  RETURN COALESCE(v_role, 'child');
END;
$$;

-- 5.2 get_my_parent_settings_status()
...
-- 5.3 verify_parent_pin(p_pin TEXT)
...
BEGIN
  ...
  -- Authorization check: ONLY parent or admin
  SELECT role INTO v_user_role FROM public.profiles WHERE id = v_user_id;
  IF v_user_role NOT IN ('parent', 'admin') THEN
    RETURN FALSE;
  END IF;
  ...
  IF NOT FOUND THEN
    INSERT INTO public.parent_settings (user_id) VALUES (v_user_id);
    v_failed := 0;
    v_locked_until := NULL;
    v_pin_hash := NULL;
  END IF;
...
-- 5.4 update_parent_pin(p_new_pin TEXT, p_old_pin TEXT DEFAULT NULL)
...
-- 5.6 increment_video_view_count(p_video_id UUID)
...
BEGIN
  ...
  -- Does NOT touch watch_history table at all
  WITH upsert AS (
...
-- Role escalation prevention trigger (Client cannot set role to publisher/admin)
...
AS $$
BEGIN
  -- Prevent escalation on INSERT or UPDATE
