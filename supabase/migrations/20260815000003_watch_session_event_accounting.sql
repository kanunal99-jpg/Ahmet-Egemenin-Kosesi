...
-- 4. Heartbeat RPC with Atomic Temporal Event Insertion (CRIT-43 & CRIT-46)
...
BEGIN
  ...
  v_delta_seconds := LEAST(v_raw_delta, 10);
  v_event_end := v_event_start + (v_delta_seconds || ' seconds')::INTERVAL;

  -- 7. Insert temporal playback segment into event ledger (CRIT-46)
  ...
  -- 8. Monotonic duration increment in session metadata capped at max daily limit (43200)
...
-- 5. Finalize RPC with Trailing Event Ledger Ingestion (CRIT-40, CRIT-44, CRIT-46)
...
BEGIN
  ...
  -- 4. CRIT-44 & CRIT-46: Server-Authoritative Duration & Trailing Event Ingestion
  ...
  v_trailing_delta := LEAST(v_raw_trailing, 10);
  v_event_end := v_event_start + (v_trailing_delta || ' seconds')::INTERVAL;

  -- Insert trailing event segment if any positive delta exists
  IF v_trailing_delta > 0 THEN
    INSERT INTO public.watch_history_session_events (
      session_id,
      user_id,
      video_id,
  ...
  -- 6. Finalize session metadata atomically
  UPDATE public.watch_history_sessions
  SET ended_at = v_now,
