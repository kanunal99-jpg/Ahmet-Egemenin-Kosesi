-- Canonical security reassertion for sensitive parent/video RPCs.
-- Idempotent: aligns live definitions and DCL with the audited security contract.
...
BEGIN
  ...
  END IF;
  SELECT pin_hash, failed_attempts, locked_until INTO v_pin_hash, v_failed, v_locked_until
  ...
  IF extensions.crypt(COALESCE(p_pin, ''), v_pin_hash) = v_pin_hash THEN
    UPDATE public.parent_settings SET failed_attempts = 0, locked_until = NULL, updated_at = NOW() WHERE user_id = v_user_id;
...
BEGIN
  ...
  SELECT id, user_id, failed_attempts, locked_until, pin_hash, daily_time_limit_minutes, allowed_categories, bedtime_start, bedtime_end, created_at, updated_at INTO v_rec
  FROM public.parent_settings WHERE user_id = v_user_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('id', NULL, 'user_id', v_user_id, 'failed_attempts', 0, 'locked_until', NULL, 'is_locked', FALSE, 'has_pin', FALSE, 'daily_time_limit_minutes', NULL, 'allowed_categories', NULL, 'bedtime_start', NULL, 'bedtime_end', NULL, 'created_at', NULL, 'updated_at', NULL);
  END IF;
  RETURN jsonb_build_object('id', v_rec.id, 'user_id', v_rec.user_id, 'failed_attempts', COALESCE(v_rec.failed_attempts, 0), 'locked_until', v_rec.locked_until, 'is_locked', (v_rec.locked_until IS NOT NULL AND v_rec.locked_until > NOW()), 'has_pin', (v_rec.pin_hash IS NOT NULL AND v_rec.pin_hash <> ''), 'daily_time_limit_minutes', v_rec.daily_time_limit_minutes, 'allowed_categories', v_rec.allowed_categories, 'bedtime_start', v_rec.bedtime_start, 'bedtime_end', v_rec.bedtime_end, 'created_at', v_rec.created_at, 'updated_at', v_rec.updated_at);
END;
$$;
...
BEGIN
  ...
  SELECT pin_hash, failed_attempts, locked_until INTO v_current_hash, v_failed, v_locked_until FROM public.parent_settings WHERE user_id = v_user_id FOR UPDATE;
  v_failed := COALESCE(v_failed, 0);
  ...
  IF v_current_hash IS NOT NULL AND v_current_hash <> '' THEN
    ...
    END IF;
  END IF;
  UPDATE public.parent_settings SET pin_hash = extensions.crypt(p_new_pin, extensions.gen_salt('bf')), failed_attempts = 0, locked_until = NULL, updated_at = NOW() WHERE user_id = v_user_id;
  IF NOT FOUND THEN
    INSERT INTO public.parent_settings (user_id, pin_hash, failed_attempts, locked_until, updated_at) VALUES (v_user_id, extensions.crypt(p_new_pin, extensions.gen_salt('bf')), 0, NULL, NOW());
  END IF;
  RETURN TRUE;
END;
$$;

CREATE OR REPLACE FUNCTION public.update_parent_settings(p_daily_time_limit_minutes INT DEFAULT NULL, p_allowed_categories TEXT[] DEFAULT NULL, p_bedtime_start TEXT DEFAULT NULL, p_bedtime_end TEXT DEFAULT NULL)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
DECLARE v_user_id UUID := auth.uid(); v_user_role TEXT;
BEGIN
  ...
  UPDATE public.parent_settings SET daily_time_limit_minutes = p_daily_time_limit_minutes, allowed_categories = p_allowed_categories, bedtime_start = p_bedtime_start, bedtime_end = p_bedtime_end, updated_at = NOW() WHERE user_id = v_user_id;
  IF NOT FOUND THEN
    INSERT INTO public.parent_settings (user_id, daily_time_limit_minutes, allowed_categories, bedtime_start, bedtime_end) VALUES (v_user_id, p_daily_time_limit_minutes, p_allowed_categories, p_bedtime_start, p_bedtime_end);
  END IF;
  RETURN TRUE;
END;
$$;
...
SET search_path = public, pg_temp
AS $$
DECLARE v_user_id UUID := auth.uid(); v_video_exists BOOLEAN; v_should_increment BOOLEAN := FALSE;
BEGIN
  IF v_user_id IS NULL THEN RETURN; END IF;
  SELECT EXISTS (SELECT 1 FROM public.videos WHERE id = p_video_id AND is_deleted = false) INTO v_video_exists;
  IF NOT v_video_exists THEN RETURN; END IF;
  WITH upsert AS (
    INSERT INTO public.video_view_cooldowns (video_id, user_id, last_counted_at, created_at) VALUES (p_video_id, v_user_id, NOW(), NOW())
    ON CONFLICT (video_id, user_id) DO UPDATE SET last_counted_at = NOW()
    WHERE video_view_cooldowns.last_counted_at <= (NOW() - INTERVAL '30 minutes') RETURNING 1
  ...
  IF v_should_increment THEN
    ...
    UPDATE public.videos SET view_count = view_count + 1 WHERE id = p_video_id AND is_deleted = false;
  END IF;
END;
$$;
