    ...
    <MainLayout>
      ...
      <section className="space-y-5 mb-9" aria-label="İçerik filtreleri">
        <div className="relative max-w-2xl">
          ...
          <input type="text" placeholder="Video veya konu ara..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-3.5 rounded-2xl bg-white border border-slate-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-600/30 focus:border-blue-600 text-sm font-medium transition-all" />
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
          ...
          {visibleCategories.map((category) => (
            <button key={category.id} onClick={() => setSelectedCategory(category.id)} aria-pressed={selectedCategory === category.id} className={`px-4 py-2.5 rounded-xl font-bold text-xs shrink-0 transition-all border ${selectedCategory === category.id ? 'bg-slate-900 text-white border-slate-900 shadow-sm' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50 hover:border-slate-300'}`}>
              {category.title}
            </button>
          ))}
        </div>
